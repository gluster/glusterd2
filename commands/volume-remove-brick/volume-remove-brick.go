// Package volumeremovebrick implements the volume remove-brick command for GlusterD
package volumeremovebrick
...
// for the volume remove-brick command
type Command struct {
}

func (c *Command) volumeRemoveBrick(w http.ResponseWriter, r *http.Request) {

	var msg volume.VolRemoveBrickRequest
	...
	//TODO : Add logic to match the existing bricks and then remove
...
// Routes returns command routes to be set up for the volume remove-brick command.
func (c *Command) Routes() rest.Routes {
	return rest.Routes{
		// VolumeRemoveBrick
		rest.Route{
			Name:        "VolumeRemoveBrick",
			Method:      "POST",
			Pattern:     "/volumes/remove-brick",
			HandlerFunc: c.volumeRemoveBrick},
	}
}
